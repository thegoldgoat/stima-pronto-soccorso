...
class TestWaitingQueue(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patient1 = Patient('0000', GaussGenerator(
            1.5, 2), ExponentialGenerator(1.2), 1, 3600)
        self.patient2 = Patient('0001', GaussGenerator(
            2, 1), ExponentialGenerator(0.8), 1, 5000)
        self.patient3 = Patient('0002', GaussGenerator(
            1, 1.3), ExponentialGenerator(2), 2, 5000)
        self.patient4 = Patient('0003', GaussGenerator(
            3, 1), ExponentialGenerator(3), 4, 2000)
        self.patient5 = Patient('0004', GaussGenerator(
            1, 2), ExponentialGenerator(3), 0, 3000)
    ...
    def test_push(self):
        ...
        self.assertEqual(
            id(self.x.priority_queues[1].heap[0]), id(self.patient1))
        self.assertEqual(
            id(self.x.priority_queues[1].heap[1]), id(self.patient2))
        self.assertEqual(
            id(self.x.priority_queues[2].heap[0]), id(self.patient3))
    ...
    def test_create_copy_and_generate(self):
        ...
        for queue in self.x.priority_queues:
            for patient in queue.heap:
                self.assertEqual(
                    patient.id, y.priority_queues[num_queue].heap[i].id)
                self.assertEqual(
                    patient.emergency_code, y.priority_queues[num_queue].heap[i].emergency_code)
                self.assertEqual(
                    patient.arrival_time, y.priority_queues[num_queue].heap[i].arrival_time)
                i += 1
            i = 0
            num_queue += 1
